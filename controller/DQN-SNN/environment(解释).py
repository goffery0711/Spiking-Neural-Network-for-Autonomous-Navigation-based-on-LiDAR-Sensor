...
# Normal Probability Distribution Function：依据正态分布/高斯分布公式
...
class VrepEnvironment():
	def __init__(self,speed,turn,resolution,reset_distance, rate, dvs_queue, resize_factor, crop):
		...
		self.fifo = deque(np.zeros((dvs_queue, resolution[0]*resolution[1])))  #deque两端都可以操作的队列，与list相似;resolution=[32,16];(10，512）
	...
	def getDistance(self,p): # Calculate robot position and distance
		...
		if p[0] < self.scale*self.v1:
			r = np.linalg.norm(p[:2]-self.c1)  #默认求距离
			delta_y = p[1] - self.c1[1]
			if self.outer:
				a = abs(math.acos(delta_y / r)/math.pi)  #反三角余弦，算出所占比例
	...
	def getState(self): # Translate DVS data from FIFO queue into state image
	    ...
	    for state in list(self.fifo): # For every DVS frame in FIFO queue
	    	for i in range(len(state)//2): # For every event in DVS frame；  //表示两数相除，向下取整，例如8 // 5 结果是1
	    		idx = (state[i*2+1]//self.resize_factor)*self.resolution[0] + state[i*2]//self.resize_factor
	    		new_state[idx] += 1
	    return new_state[(self.crop[1]*self.resolution[0]):-(self.crop[0]*self.resolution[0])]  #(6*32 : -10*32)
