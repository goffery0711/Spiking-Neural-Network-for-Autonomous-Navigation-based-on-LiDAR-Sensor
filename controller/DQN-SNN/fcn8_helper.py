...
def save_inference_samples(test_img, sess, image_shape, logits, keep_prob, input_image):
    ...
    new_state = np.zeros((16, 8), dtype=int)
    res = 8
    for i in range(11, im_softmax.shape[0]/res+1):   # 160/8 = 20   (11, 21)
        ...
        if pos[1][find_gt].size:
            ...
            if max_pix < im_softmax.shape[1] - 6:
                x_max = int(16.0 * max_pix / im_softmax.shape[1])  # find max_pos in x axis
                new_state[x_max, y] += 1
            if min_pix > 5:
                x_min = int(16.0 * min_pix / im_softmax.shape[1])    # find min_pos in x axis
                new_state[x_min, y] += 1
