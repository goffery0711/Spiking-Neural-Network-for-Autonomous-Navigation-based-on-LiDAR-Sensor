...
class VrepEnvironment():
	def __init__(self):
		...
		self.d2_outer_square = 2 * math.pi * (self.r1_outer - 0.25) * 0.25
		self.d3_outer_square = 5.0
		self.d4_outer_square = 2 * math.pi * (self.r1_outer - 0.25) * 0.25
		self.d5_outer_square = 5.0
		self.d6_outer_square = 2 * math.pi * (self.r1_outer - 0.25) * 0.25
		self.d7_outer_square = 5.0
		self.d8_outer_square = 2 * math.pi * (self.r1_outer - 0.25) * 0.25
		self.d1_inner_square = 5.0
		self.d2_inner_square = 2 * math.pi * (self.r1_inner + 0.25) * 0.25
		self.d3_inner_square = 5.0
		self.d4_inner_square = 2 * math.pi * (self.r1_inner + 0.25) * 0.25
		self.d5_inner_square = 5.0
		self.d6_inner_square = 2 * math.pi * (self.r1_inner + 0.25) * 0.25
		self.d7_inner_square = 5.0
		self.d8_inner_square = 2 * math.pi * (self.r1_inner + 0.25) * 0.25
		...
		# x > 7.5
			...
			if p[1] < self.scale * self.v1:
				...
				if self.outer:
					...
					a = 1. - abs(math.asin(delta_x / r) / (0.5 * math.d4_outer_squarepi))
					position = self.d1_inner + a * self.d2_inner
					distance = r - self.r1_inner
					return distance, position
			# straight for 2.5 < y < 7.5
			...
			if p[1] < self.scale * self.v1:
				if self.outer:
					...
					position = abs(p[0] - self.v1)
					return distance, positiond4_outer_square
				else:
	...
	def getDistance_square(self, p):
		...
		if p[0] < self.scale * self.v1:
			...
			if p[1] < self.scale * self.v1:
				...
				delta_y = p[1] - self.c1[1]
				if self.outer:
					a = abs(math.asin(delta_y / r) / (0.5 * math.pi))  # get ratio of 90 degrees
